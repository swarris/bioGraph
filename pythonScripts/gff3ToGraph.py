...
def createSequenceStructure():
    ...
    seq = session.run("match (a:sequence_{}) return a.name as name;".format(organism))
    for s in seq:
        prevStart = 0
...
for l in GFF:
    ...
    if len(l)>= 9:
        ...
        if node in ["gene", "transcript", "CDS"]:
            if node == "gene":
                node = "gene_" + organism
            session.run("create (n:{} {{sequence: '{}', start: {}, end: {}, geneID: '{}'}});".format(node, chr, start, end, geneID))
            #print("create (n:{} {{sequence: '{}', start: {}, end: {}, geneID: '{}'}});".format(node, chr, start, end, geneID))
            sequences.add(chr)
